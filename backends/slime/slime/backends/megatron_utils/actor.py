...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def _get_rollout_data(self, rollout_data_ref: Box) -> RolloutBatch:
        ...
        for key in ["rollout_log_probs", "teacher_log_probs", "native_advantages", "native_token_weights"]:
    ...
    def train_actor(self, rollout_id: int, rollout_data: RolloutBatch) -> None:
            ...
            if self.args.compute_advantages_and_returns:
                ...
                if (
                    ...
                    if "proximal" not in self.weights_backuper.backup_tags:
                        if is_megatron_main_rank():
                            logger.info("Initializing proximal policy from current actor at rollout_id %s", rollout_id)
                ...
                if getattr(self.args, "enable_trajectory_replay", False) and rollout_data.get("sil_sample_flags"):
                    try:
                        flags = rollout_data["sil_sample_flags"]
                        precomputed = rollout_data.get("sil_precomputed_advantages", [])
                        advantages = rollout_data.get("advantages", [])
                        overridden = 0
                        for idx, flag in enumerate(flags):
                            if flag and idx < len(precomputed) and precomputed[idx] is not None and idx < len(advantages):
                                value = precomputed[idx]
                                if hasattr(value, "to") and hasattr(advantages[idx], "device"):
                                    value = value.to(device=advantages[idx].device, dtype=advantages[idx].dtype)
                                advantages[idx] = value
                                overridden += 1
                        if overridden and is_megatron_main_rank():
                            logger.info("Overrode advantages for %d SIL samples", overridden)
        ...
        if self.args.loss_type == "decoupled_policy_loss":
            self._backup_model_weights("proximal")
